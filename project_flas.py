...
spell_word = random_word() #global variable
...
def create_spectrogram():
	for filename in os.listdir('test_data'):
		audio= 'test_data/harshu_file.wav'
		...
		plt.tight_layout()
		plt.savefig('test_data/test/harshu_file.png')
		plt.clf()
		plt.close()
...
def model():
	...
	#word = random_word()
	index = labels[spell_word]
	percent = pred[0][index]*100
	percent = float(percent)
	return percent,spell_word
...
def predicting():
	k.clear_session()
	percent,ran_word= model()
	k.clear_session()
	return render_template('test.html',val = percent,words=ran_word )
